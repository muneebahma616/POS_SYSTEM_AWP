...
function onlyNumberInput(val, id) {
...
function addbtn() {
  ...
  const totalGrams = parseFloat(totalGramsInput.value);
...
function edit(num, price, totalGrams, purity) {
  ...
  // document.getElementById('descriptionLabel').classList.add('up')
...
function plusplus() {
  ...
  for (var i = 0; i < table.rows.length; i++) {
    sumVal += parseFloat(table.rows[i].cells[4].innerText);
  }
